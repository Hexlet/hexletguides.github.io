...
Lisp оказался удачным изобретением. За прошедшие годы люди изобрели тысячи языков, которые оказались никому не нужны, в отличие от Lisp, который развивается до сих пор. Так как разработкой интерпретаторов Lisp занимались разные команды, у них получались разные версии языка.

Такое случается с популярными языками, имеющими долгую историю. Разные версии языка называют его *диалектами*. Услышав фразу «язык Clojure является диалектом Lisp», вы сразу понимаете, что Clojure — одна из современных разновидностей языка Lisp.

Где мы можем встретить Lisp? В популярном текстовом редакторе Emacs, который почти полностью написан на диалекте Emacs Lisp. На курсах Массачусетского технологического института, где долгое время преподавали программирование, используя диалект Scheme. При разработке фронтенда, где для реализации сложной логики вместо JavaScript применяют диалект Lisp, который называется ClojureScript.

Для того, чтобы оценить разницу между машинным кодом и «почти человеческим языком», посмотрим, как наша программы выглядит на Lisp:
...
<<<<<<< HEAD
Есть интерпретаторы для таких популярных языков программирования, как Python и JavaScript. Эти языки хорошо подходят не только для работы, но и для обучения. Здесь есть один небольшой секрет — оказывается, интерпретируемые языки изучать легче. Чуть позже мы узнаем, почему это так.
=======
...
>>>>>>> 12b18abfa5c67e3b1254d409dc04f05a8fe9b555
