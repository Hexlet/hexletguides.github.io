...
В Месопотамии, примерно 3500 лет назад, гончары использовали шифрование для записи формул и рецептов на глиняных табличках, чтобы уберечь их от кражи конкурентами. Политики, для зашифровки писем, пользовались шифром с алфавитной подстановкой, тем самым страховались от перехвата, например, своих приказов во время боевых действий.  

Храним мы информацию у себя или передаём из точки А в точку Б, нам небходимо сохранить информацию в безопасности.
...
Это очень простой для взлома шифр, для его расшифровки не предётся использовать вычислительные мощности компьютера, нам достаточно карандаша и литса бумаги для решения головоломки. Но шифр можно улучшить, применив к сообщению более продвинутый метод, который самостоятельно модифицирует криптоалгоритм после каждого выполнения, и на каждой итерации получаются разные результаты. Такой метод называется —**полиморфизм.**
...
Сегодня, чтобы обеспечить безопасность огромной системы электронных коммуникаций и торговли, классические методы уже не годятся. Ведь теперь приходится работать не только с частными случайми хранения или передачи информации, но и с огромными объёмами данных.

В основном используются три криптографических метода, каждый из которых имеет преимущества и недостатки.

### **Хеширование**
 Это алгоритм, который преобразовывает информацию в уникальный набор символов, но не шифрует данные.
