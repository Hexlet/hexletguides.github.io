...
import RSSFeedPostContent from '../components/RSSFeedPostContent';
import { i18n } from '../next-i18next.config';
import cfg from '../data/config';

const makeHref = (pathname: string | null, locale: string): string => {
  const parts = ['/'];
...
const formatNameToHeader = (name: string): string => {
  return name.split('-').map(capitalize).join(' ');
};

const readPost = async (filePath: string, basePath: string, locale: string) => {
...
export const getPublishedPosts = async (locale: string) => {
...
export const getPostsList = async (locale: string) => {
  const posts = await getPublishedPosts(locale);

  return posts.filter(({ hidden = false }) => !hidden).reverse();
};

export const findPost = async (name: string, locale: string) => {
  const posts = await getPublishedPosts(locale);
...
export const generateRssFeed = async (locale: string) => {
  const posts = await getPublishedPosts(locale);
...
export const generateSitemap = async (locale: string) => {
  const posts = await getPublishedPosts(locale);
...
const getMatches = (content: string) => {
  const h2Regex = /^## (.*$)/gim;
...
const addLinksToContent = (matches: string[] | null, content: string) => {
  if (matches) {
...
export const getPostAwailableLocales = async (name: string, locale: string, locales: string[]) => {
