...
Когда мы выполняем команду _run_ `docker run <image name>`, то Docker проверяет наличие указанного образа локально и скачивает его по необходимости. Список образов уже скачанных на компьютер, можно посмотреть командой `docker images`. На практике имя образа указывается множеством разных способов: _nginx_, _nginx:latest_, _nginx:1.2_, _nginx:1_, _hashicorp/nginx_, _etsy/nginx:3_. Канонической формой является _etsy/nginx:3_, что расшифровывается так _<account name>/<image name>:<tag>_.

docker cloud pull push tags remove images
